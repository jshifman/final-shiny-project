...
# TP & TPA from the NBA championship for given year
tpa_and_tp <- function(champs, runnerup, year) {
  ret <- list()
  tpa_champs <- champs %>%
    select(Year, TPA) %>%
    filter(Year == year) %>%
...
# Average # pts a team scored in a single round of the NBA championship
total_points <- function(champs, runnerup, year1, year2) {
  ret <- list()
  ...
  pts_runnerups_rows <- runnerup %>%
    ...
    filter(Year >= year1 & Year <= year2) %>%
    nrow()
  ret$pts <- round(((pts_champs_total / pts_champs_rows)
